...
command = "unzip -o " + dirname(snakemake.input.trimmed) + "/\"*fastqc.zip\"" + " -d " + str(snakemake.params.folder)
f = open(log_filename, "at")
f.write("\n##\n## COMMAND: " + command + "\n")
f.close()
shell(command)

command = "grep \"Total Sequences\" " + str(snakemake.params.folder) + "/" + snakemake.wildcards.sample + ".*/*.txt >> " + snakemake.output.summary + " 2>> " + log_filename
f = open(log_filename, "at")
f.write("\n##\n## COMMAND: " + command + "\n")
f.close()
shell(command)
